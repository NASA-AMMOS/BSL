...
int mock_bpa_eid_init(BSL_HostEID_t *eid, void *user_data _U_)
{
    CHKERR1(eid);
    assert(eid != NULL);
...
void mock_bpa_eid_deinit(BSL_HostEID_t *eid, void *user_data _U_)
{
    CHKVOID(eid);
    if (eid->handle)
    {
...
int mock_bpa_get_secsrc(BSL_HostEID_t *eid, void *user_data)
{
    ...
    // return mock_bpa_eid_from_text(eid, "ipn:0.0", user_data);
}
...
int mock_bpa_eid_to_text(string_t out, const BSL_HostEID_t *eid, void *user_data _U_)
{
    ...
    switch (obj->scheme)
    {
        case BSL_MOCK_EID_IPN:
        {
            ...
            {
                case 2:
                    string_printf(out, "ipn:%" PRIu64 ".%" PRIu64, (ipn->auth_num << 32) | ipn->node_num, ipn->svc_num);
                    break;
                case 3:
