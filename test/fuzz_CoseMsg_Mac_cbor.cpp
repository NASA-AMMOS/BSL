...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
...

    {
        ...
        int res = BSL_CBOR_Decode(&in_buf, (BSL_CBOR_Decode_f)&BSLX_CoseMsg_Mac_Decode, &msg);
    ...
    if (!retval)
    {
        int res = BSL_CBOR_Encode_Twopass(&out_buf, (BSL_CBOR_Encode_f)&BSLX_CoseMsg_Mac_Encode, &msg);
        if (BSL_SUCCESS != res)
        {
            retval = -1;
        }
