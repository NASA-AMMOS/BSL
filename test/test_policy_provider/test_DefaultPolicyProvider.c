...
void test_SamplePolicyProvider_WildcardPolicyRuleVerifiesBIB(void)
{
    ...
    BSLP_PolicyPredicate_Init(&predicate,
                             BSL_POLICYLOCATION_APPIN,
                             GetEIDPatternFromText("*:**"), 
                             GetEIDPatternFromText("*:**"), 
                             GetEIDPatternFromText("*:**"));

    // Create a rule to verify the bundle contains a BIB block covering the payload
    BSLP_PolicyRule_t rule;
    BSLP_PolicyRule_Init(&rule, "Confirm bundle has BIB protecting payload", predicate, 1, BSL_SECROLE_VERIFIER, BSL_SECBLOCKTYPE_BIB, BSL_BLOCK_TYPE_PAYLOAD);
    
    // Now evaluate the rule to get as a SecOper
    // This populates it with actual parameters.
    BSL_SecOper_t sec_oper;
    TEST_ASSERT_EQUAL(0, BSLP_PolicyRule_EvaluateAsSecOper(&rule, &sec_oper, LocalTestCtx.mock_bpa_ctr.bundle, BSL_POLICYLOCATION_APPIN));
