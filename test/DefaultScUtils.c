...
void BSL_TestUtils_InitBIB_AppendixA1(BIBTestContext *context, BSL_SecRole_e role, const char *key_id)
{
    ...
    BSL_SecParam_InitTextstr(&context->param_test_key, BSLX_BIB_OPT_KEY_ID, key_id);
    BSL_SecParam_InitUint64(&context->param_scope_flags, BSLX_BIB_OPT_SCOPE, 0);
    BSL_SecParam_InitUint64(&context->param_sha_variant, BSLX_BIB_OPT_SHA_VARIANT, RFC9173_BIB_SHA_HMAC512);
    BSL_SecParam_InitUint64(&context->use_key_wrap, BSLX_BIB_OPT_USE_KEY_WRAP, 0);
    ...
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_sha_variant);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_scope_flags);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_test_key);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->use_key_wrap);
}
...
void BSL_TestUtils_InitBCB_Appendix2(BCBTestContext *context, BSL_SecRole_e role)
{
    ...
    BSL_SecParam_InitUint64(&context->param_scope_flags, BSLX_BCB_OPT_SCOPE, 0);
    BSL_SecParam_InitTextstr(&context->param_test_key_id, BSLX_BCB_OPT_KEY_ID, RFC9173_EXAMPLE_A2_KEY);
    BSL_SecParam_InitUint64(&context->param_aes_variant, BSLX_BCB_OPT_AES_VARIANT, RFC9173_BCB_AES_VARIANT_A128GCM);
    ...
    BSL_SecParam_InitUint64(&context->use_key_wrap, BSLX_BCB_OPT_USE_KEY_WRAP, 1);
    ...
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_init_vec);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_aes_variant);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_wrapped_key);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->use_key_wrap);
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_scope_flags);
    if (role != BSL_SECROLE_SOURCE)
    {
        BSL_SecOper_AppendOption(&context->sec_oper, &context->param_auth_tag);
    }
    BSL_SecOper_AppendOption(&context->sec_oper, &context->param_test_key_id);
}
...
RFC9173_A1_Params BSL_TestUtils_GetRFC9173_A1Params(const char *key_id)
{
    ...
    BSL_SecParam_InitTextstr(&params.test_key_id, BSLX_BIB_OPT_KEY_ID, key_id);
    BSL_SecParam_InitUint64(&params.use_key_wrap, BSLX_BIB_OPT_USE_KEY_WRAP, 0);
    return params;
}
