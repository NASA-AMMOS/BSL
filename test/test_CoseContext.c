...
void test_CoseSc_InvalidOptions_Source(void)
{
    ...
    set_CoseSc_InvalidOptions_Source_baseline(&sec_oper);
    BSLB_VariantPtrMap_erase(sec_oper._options, BSLX_COSESC_OPTION_KEY_ID);
    TEST_ASSERT_FALSE(BSLX_CoseSc_Validate(&LocalTestCtx.bsl, &LocalTestCtx.mock_bpa_ctr.bundle_ref, &sec_oper));

    set_CoseSc_InvalidOptions_Source_baseline(&sec_oper);
    BSLB_VariantPtrMap_erase(sec_oper._options, BSLX_COSESC_OPTION_TGT_ALG);
    TEST_ASSERT_FALSE(BSLX_CoseSc_Validate(&LocalTestCtx.bsl, &LocalTestCtx.mock_bpa_ctr.bundle_ref, &sec_oper));
...
void test_AppendixA_Example1_BIB_Source(void)
{
    ...
    BSL_Crypto_KeyHandle_t keyhandle;
    {
        ...
        BSL_Variant_SetInt64(MockBPA_KeyStore_SetKeyParameter(keyhandle, BSLX_COSEMSG_KEY_PARAM_ALG),
                             BSLX_COSEMSG_ALG_HMAC_SHA_384_384);
    ...
    {
        BSL_Data_t in_buf;
        TEST_ASSERT_EQUAL(BSL_SUCCESS, BSL_Variant_GetAsBytestr(result, &in_buf));
...
void test_AppendixA_Example1_BIB_VerifyAccept(BSL_SecRole_e role, int mismatch)
{
    ...
    BSL_SecOper_t sec_oper;
    BSL_SecOper_Init(&sec_oper);
    BSL_SecOper_Populate(&sec_oper, BSLX_COSESC_CTX_ID, 1, 3, BSL_SECBLOCKTYPE_BIB, role, BSL_POLICYACTION_DROP_BUNDLE, 0);
...
void test_CCSDS_Example_Mac_VerifyAccept(BSL_SecRole_e role, int mismatch)
{
    ...
    BSL_SecOper_t sec_oper;
    BSL_SecOper_Init(&sec_oper);
    BSL_SecOper_Populate(&sec_oper, BSLX_COSESC_CTX_ID, 1, 5, BSL_SECBLOCKTYPE_BIB, role, BSL_POLICYACTION_DROP_BUNDLE, 0);
...
void test_AppendixA_Example4_BCB_Source(void)
{
    ...
    BSL_Crypto_KeyHandle_t keyhandle;
    {
                             ...
                             BSLX_COSEMSG_ALG_AES_GCM_256);
        {
            ...
            BSL_Variant_SetBytestr(MockBPA_KeyStore_SetKeyParameter(keyhandle, BSLX_COSEMSG_KEY_PARAM_BASEIV), buf);
            BSL_Data_Deinit(&buf);
        }
...
void test_AppendixA_Example4_BCB_VerifyAccept(BSL_SecRole_e role, int mismatch)
{
    ...
    BSL_SecOper_t sec_oper;
    BSL_SecOper_Init(&sec_oper);
    BSL_SecOper_Populate(&sec_oper, BSLX_COSESC_CTX_ID, 1, 3, BSL_SECBLOCKTYPE_BCB, role, BSL_POLICYACTION_DROP_BUNDLE, 0);
