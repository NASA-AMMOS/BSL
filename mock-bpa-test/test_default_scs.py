    ...
    def test_exampleA_1_acceptor_failure_key_mismatch(self):
        with sc_config_modifier(
            ...
            self._single_test(
                _TestCase(
                    input_data=EXAMPLE_A_1_WITH_BIB,
                    expected_output=".*<ERROR>.* Security operation mismatch - ASB does NOT contain block 1",
                    sec_src_eid="ipn:1.0",
...
class TestBcbAesGcm(TestAgent):
    ...
    def test_exampleA_2_acceptor_failure_wrapped_badunwrap(self):
        input_diag = """\
[_
    [7, 0, 0, [2, [1, 2]], [2, [2, 1]], [2, [2, 1]], [0, 40], 1000000],
    [12, 2, 1, 0, << [1], 2, 1, [2, [2, 1]], [[1, h'5477656C7665313231323132'], [2, 1], [3, h'69C411276FECDDC4780DF42C8A2AF89296FABF34D7FAE701'], [4, 0]], [[[1, h'EFA4B5AC0108E3816C5606479801BC04']]] >>],
    [1, 1, 0, 0, h'3A09C1E63FE23A7F66A59C7303837241E070B02619FC59C5214A22F08CD70795E73E9A']
]
"""
...
"""
    ...
    def test_exampleA_2_acceptor_failure_wrapped_length_mismatch(self):
        # The unwrap succeeds, but the key is not correct length for the AEAD alg
        input_diag = """\
[_
    [7, 0, 0, [2, [1, 2]], [2, [2, 1]], [2, [2, 1]], [0, 40], 1000000],
    [12, 2, 1, 0, << [1], 2, 1, [2, [2, 1]], [[1, h'5477656C7665313231323132'], [2, 1], [3, h'b86e19fac9a921125c62b4fad5d8040d14935ff02a9e9491d06f85b0f7eee67e'], [4, 0]], [[[1, h'EFA4B5AC0108E3816C5606479801BC04']]] >>],
    [1, 1, 0, 0, h'3A09C1E63FE23A7F66A59C7303837241E070B02619FC59C5214A22F08CD70795E73E9A']
]
"""
        ...
        self._single_test(
            _TestCase(
                input_data=input_diag,
                expected_output=".*<ERROR>.* Cipher key size needed 16 got 24",
                sec_src_eid="ipn:1.0",
                policy_config="data/default-scs/policy-exA.2-accept.json",
                bundle_dest_loc=BundleDestLoc.APPIN,
                key_set="data/default-scs/keyset-1.json",
